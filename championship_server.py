...
function syncCoreReadiness(){q.caseReady=['project_name','lat','lon','state','pbt','district','landuse','category','activity','development','ratio'].every(k=>{const e=document.querySelector(`#cs-${k}`);return !!e&&String(e.value||'').trim()!==''});}
...
function boot(){wrapCtor('GridLayer');wrapCtor('GeoJSON');wrapCtor('FeatureGroup');sync();syncCoreReadiness();}
