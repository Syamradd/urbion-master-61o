...
function dismissWorkspaceBrief(){
  const brief=$('urbionWorkspaceBrief');
  if(brief) brief.remove();
}
function installBriefGuard(){
  dismissWorkspaceBrief();
  const obs=new MutationObserver(()=>dismissWorkspaceBrief());
  if(document.body) obs.observe(document.body,{childList:true,subtree:true});
  window.__URBION_BRIEF_OBSERVER__=obs;
}
...
async function loadDemo(){
  Object.entries(DEMO).forEach(([k,v])=>{if(['district','pbt','landuse2','landuse3'].includes(k))return;setV(k,v)});
  setV('state',DEMO.state); await sleep(120);
  if(await opt('district',DEMO.district)) setV('district',DEMO.district); await sleep(100);
  if(await opt('pbt',DEMO.pbt)) setV('pbt',DEMO.pbt);
  if(await opt('landuse1',DEMO.landuse1)) setV('landuse1',DEMO.landuse1); await sleep(80);
  if(await opt('landuse2',DEMO.landuse2)) setV('landuse2',DEMO.landuse2); await sleep(80);
  if(await opt('landuse3',DEMO.landuse3)) setV('landuse3',DEMO.landuse3);
  checkV('shop_frontage_verified',true); checkV('shop_office_verified',true);
  let box=$('udcDemoCase'); if(box)box.innerHTML='<strong>LIVE DEMO CASE ·</strong> Lot 11213 · Padang Semabok · Melaka Tengah · Melaka · ~1.145 ha · Commercial planning case.<span class="udc-status" id="udcDemoState">DEMO INPUTS LOADED</span><small>Map point remains demo context only; URBION does not claim authoritative parcel geometry here.</small>';
  return true;
}
function mountDemoFallback(){
  if($('urbionDemoCommand')) return true;
  const host=document.querySelector('.center')||document.querySelector('.layout')||document.body;
  if(!host)return false;
  const box=document.createElement('section'); box.id='urbionDemoCommand'; box.className='urbion-demo-command';
  box.style.cssText='border:1px solid rgba(47,225,233,.24);border-radius:12px;padding:10px;margin:0 0 8px;background:rgba(3,20,28,.96)';
