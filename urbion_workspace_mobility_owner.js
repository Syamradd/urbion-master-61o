...
function render(payload,status='QUERYING'){
 ...
 else {const j=payload.jps_rainfall||{}, w=payload.eqmp_water||{}, a=payload.air_quality||{}; const js=Array.isArray(j.stations)?j.stations:[], ws=Array.isArray(w.stations)?w.stations:[], as=Array.isArray(a.stations)?a.stations:[]; const nearest=js[0]||ws[0]||as[0]; const dist=nearest?.distance_m!=null?Math.round(nearest.distance_m):'—'; const water=ws[0]; const waterMetric=water?.ph!=null?`pH ${water.ph}`:water?.turbidity_ntu!=null?`Turbidity ${water.turbidity_ntu} NTU`:'No live water metric'; c.innerHTML=`<div class="urbion-env-head"><span class="urbion-env-title">LIVE MOBILITY / STATIONS</span><span class="urbion-env-state">${esc(j.status||status)}</span></div><div class="urbion-env-grid"><div class="urbion-env-m"><b>${js.length}</b><span>JPS STATIONS</span></div><div class="urbion-env-m"><b>${ws.length}</b><span>MYEQMS WATER</span></div><div class="urbion-env-m"><b>${as.length}</b><span>AIR STATIONS</span></div></div><div class="urbion-env-risks">${nearest?`<b>NEAREST:</b> ${esc(nearest.name||nearest.station_id||'Station')} · ${dist}m`:'<b>STATUS:</b> No station geometry returned.'}</div><div class="urbion-env-foot">JPS / JAS MyEQMS / DOE APIMS · ${esc(w.status||'SOURCE_CONTEXT')} · ${esc(waterMetric)} · SOURCE_CONTEXT · statutory verification NOT_CLAIMED</div>`}
 host.prepend(c)
}
