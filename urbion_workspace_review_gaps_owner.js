...
function modalContext(modal){
 const title=String(modal?.querySelector?.('#modalTitle')?.textContent||'').trim().toUpperCase();
 if(title==='EVIDENCE')return 'EVIDENCE';
 if(title==='DECISION')return 'DECISION';
 return modal?.classList.contains('show')?'REVIEW':'LIVE EVIDENCE';
}
function surface(context){
 ...
 rendering=true;
 try{style();target.querySelectorAll('.urbion-review-gap-surface[data-owner="canonical"]').forEach(x=>x.remove());const list=gaps();const label=context||modalContext(modal);const el=document.createElement('section');el.className='urbion-review-gap-surface'+(box?'':' urbion-review-gap-rail');el.dataset.owner='canonical';el.dataset.testid='canonical-review-gaps';el.innerHTML='<div class="rg-head"><span class="rg-title">'+esc(label)+' · REVIEW GAPS</span><span class="rg-count">'+list.length+' OPEN</span></div><p class="rg-note">Canonical evidence packet · statutory verification: '+esc(p.statutory_verification||'NOT_CLAIMED')+'. These are traceability/review boundaries, not approval decisions.</p>'+(list.length?list.map((g,i)=>'<div class="rg-item"><b>'+(i+1)+'. REVIEW</b> '+esc(g)+'</div>').join(''):'<div class="rg-empty">No unresolved evidence gaps in the canonical packet.</div>');if(box)box.appendChild(el);else target.prepend(el)}finally{rendering=false}}
window.URBION_REVIEW_GAPS={render:context=>surface(context),get count(){return gaps().length}};
