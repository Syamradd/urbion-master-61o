...
def _about() -> HTMLResponse:
    return HTMLResponse(_read(ABOUT_FILE),media_type='text/html; charset=utf-8',headers={'Cache-Control':'no-store, max-age=0, must-revalidate','X-URBION-UI':'CANONICAL-ABOUT','X-URBION-ABOUT':'ABOUT-CANONICAL-V3-PNG','X-URBION-ABOUT-SOURCE':'urbion_horizon_about.html','X-URBION-ABOUT-MASTER-FORMAT':'PNG'})
...
def welcome_background():
    if not WELCOME_BACKGROUND_FILE.is_file(): raise HTTPException(status_code=404,detail='Welcome background is missing')
...
def about()->HTMLResponse:return _about()

@app.get('/workspace',include_in_schema=False)
@app.get('/workspace-v5.html',include_in_schema=False)
def workspace()->HTMLResponse:return HTMLResponse(_workspace(),media_type='text/html; charset=utf-8',headers={'Cache-Control':'no-store, max-age=0','X-URBION-UI':'CANONICAL-V5-ISOLATED'})

def _js(path: Path,message:str)->Response:
    if not path.is_file(): return Response(message,status_code=500,media_type='text/plain; charset=utf-8')
...
def about_master_png():
    if not ABOUT_MASTER_FILE.is_file(): raise HTTPException(status_code=404,detail='About master PNG is missing')
...
def about_master_legacy():
    ...
    if not target.is_file(): raise HTTPException(status_code=404,detail='Legacy About master is missing')
