...
frontend owner. It exposes the canonical About raster asset, normalizes a legacy
UI payload shape before assessment, and adds a narrow redirect for critical
Melaka i-Plan land-use tiles when the WMS path is unavailable.
"""
...
async def _urbion_v4_compatibility(request: Request, call_next):
    ...
    if request.method == "GET" and request.url.path == "/map/wms":
        layer = request.query_params.get("layers", "")
        service = _CRITICAL_IPLAN_ARCGIS.get(layer)
        if service:
