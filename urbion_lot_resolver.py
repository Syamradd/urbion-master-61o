...
def _query_layer(service: str, lat: float | None = None, lon: float | None = None, where: str = "1=1", return_geometry: bool = True, max_records: int = 20) -> dict[str, Any]:
    ...
    payload = _json(url)
    if payload.get("status") == "QUERY_UNAVAILABLE":
        return {**payload, "features":[]}
    if payload.get("error"):
        return {"status":"QUERY_UNAVAILABLE", "error":payload["error"], "url":url, "features":[]}
...
def _lot_identity(attrs: dict[str, Any]) -> dict[str, Any]:
    return {
        "lot_no": _first(attrs, "LOT", "lot_no", "NO_LOT"),
        "upi": _first(attrs, "UPI", "upi"),
        "state": _first(attrs, "NEGERI", "negeri"),
        "district": _first(attrs, "DAERAH", "daerah"),
        "mukim": _first(attrs, "MUKIM", "mukim"),
        "section": _first(attrs, "SEKSYEN", "seksyen"),
        "area": _first(attrs, "KELUASAN", "luas"),
    }


def _same_identity(a: dict[str, Any], b: dict[str, Any]) -> bool:
    a_upi, b_upi = _clean(a.get("upi")), _clean(b.get("upi"))
    if a_upi and b_upi:
        return a_upi == b_upi
    a_lot, b_lot = _clean(a.get("lot_no")), _clean(b.get("lot_no"))
    return bool(a_lot and b_lot and a_lot == b_lot)
...
def resolve_lot(lat: float | None = None, lon: float | None = None, state: str = "Melaka", lot_no: str | None = None, upi: str | None = None, project_reference: dict[str, Any] | None = None) -> dict[str, Any]:
    if state not in STATE_CODES:
        return {"status":"UNSUPPORTED_STATE", "state":state, "decision_safe":False}
    code = STATE_CODES[state]
    service = f"LOT_{code}"
    if (lat is None) != (lon is None):
        raise ValueError("lat and lon must be supplied together")
    if lat is None and not (lot_no or upi):
        raise ValueError("provide site coordinates, lot_no, or upi")

    query = _query_layer(service, lat, lon, where="1=1" if lat is not None else "1=1", return_geometry=True, max_records=20)
    ...
    if not features:
        return {
            "status":"EVIDENCE_GAP" if query["status"] == "QUERY_UNAVAILABLE" else "NO_FEATURE",
            "state":state,
            "requested":requested,
            "source":{"id":"iplan","service":service,"layer":"Lot Melaka" if state == "Melaka" else f"Lot {state}","url":query.get("url"),"evidence":"SOURCE_CONTEXT"},
            "jupem_verification":{"provider":"JUPEM MyLot","url":MYLOT_URL,"status":"MANUAL_VERIFICATION_REQUIRED","decision_safe":False},
            "decision_safe":False,
        }

    candidates=[]
    for feature in features:
        identity=_lot_identity(_attrs(feature))
        matches=[]
    ...
    selected=max(candidates, key=lambda x: len(x["matches"]))
    corroboration=[]
    if selected["matches"]: corroboration.extend(selected["matches"])
    status="VERIFIED_CANDIDATE" if any(x in selected["matches"] for x in ("LOT","UPI")) else "SOURCE_CONTEXT"
    if project_reference and not selected["reference_match"]:
        status="EVIDENCE_GAP"
    return {
        "status":status,
        "state":state,
        ...
        "sources":[
            {"id":"iplan","name":"PLANMalaysia i-Plan LOT","service":service,"evidence":"SOURCE_CONTEXT","geometry":"LIVE_ARCGIS_REST"},
            {"id":"jupem-mylot","name":"JUPEM MyLot","url":MYLOT_URL,"evidence":"MANUAL_VERIFICATION","status":"REFERENCE / VERIFICATION CHANNEL"},
        ],
        "authority_model":"OFFICIAL_SOURCE_CANDIDATE; JUPEM CADASTRAL VERIFICATION REQUIRED FOR AUTHORITATIVE CLAIM",
        "decision_safe":False,
        "disclaimer":"A public i-Plan lot geometry is a strong official planning/GIS anchor, but URBION does not silently label it as JUPEM-authoritative cadastral evidence. Verify against JUPEM MyLot/eKadaster or an authorised cadastral product before statutory reliance.",
    }
