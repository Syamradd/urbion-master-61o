...
FILES = [
    ...
    "urbion_layer_runtime_fix.js", "urbion_workspace_canonical_ui.js",
]
IDS = [
...
def main() -> None:
    ...
    if any(p < 0 for p in positions) or positions != sorted(positions):
        fail("canonical script order must be bridge -> runtime -> layer -> canonical-ui")
    if "/urbion_workspace_final.js" in landing and "scripts =" in landing:
        ...
        if "/urbion_workspace_final.js" in inject:
            fail("legacy final.js must not be injected by canonical workspace route")
    ...
    if "__URBION_RUNTIME_CANONICAL_V5_COMPAT__" not in runtime:
        fail("runtime must declare canonical V5 compatibility bootstrap")
    ok("runtime compatibility-only contract verified")

    for control_id in RUNTIME_IDS:
        if control_id not in workspace and control_id not in texts["urbion_workspace_canonical_ui.js"]:
            # Utility owners may render these controls, so require presence in the runtime surface overall.
            if control_id not in all_source:
                fail(f"runtime utility control missing: {control_id}")
    ok("runtime utility controls present in canonical surface")

    canonical = texts["urbion_workspace_canonical_ui.js"]
    for token in (
        "__URBION_CANONICAL_UI_V1__", "runAnalysisCanonical", "baseSwitch",
        "syncSite", "bindSearch", "URBION_LAYER_MANAGER",
    ):
        if token not in canonical:
            fail(f"canonical owner token missing: {token}")
    ...
    for name in (
        "urbion_workspace_final.js", "urbion_workspace_bridge.js",
        "urbion_workspace_runtime.js", "urbion_layer_runtime_fix.js",
        "urbion_workspace_canonical_ui.js",
    ):
        text = texts[name]
