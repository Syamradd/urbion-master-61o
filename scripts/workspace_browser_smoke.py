...
def prepare_ready_case(page):
    ...
    ready = page.evaluate("""()=>{
      ...
      const controls=[pick('project / site name'),pick('state'),pick('district'),pick('local authority'),document.querySelector('#site_lat'),document.querySelector('#site_lon'),pick('development type'),pick('development class'),document.querySelector('#landuse1'),document.querySelector('#landuse2'),document.querySelector('#landuse3')].filter(Boolean);
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        def on_request(request):
            if request.url.endswith("/workstation/analysis"): analysis_requests.append(request.url)
            elif request.url.endswith("/copilot/explain"): copilot_requests.append(request.url)
            elif request.url.endswith("/what-if"): whatif_requests.append(request.url)
        ...
        try:
            ...
            for ident in ["landuse1","landuse2","landuse3"]: check(page.locator(f"#{ident}").count()==1, f"{ident.upper()} control unique")
            ...
            sel_districts=usable_options(district); sel_pbts=usable_options(pbt)
            check(len(sel_districts)>=5,"State change populates District hierarchy"); check(len(sel_pbts)>=10,"State change populates authoritative PBT catalogue")
            district.select_option(label=sel_districts[0]); page.wait_for_timeout(350); check(len(usable_options(mukim))>=1,"District change populates Mukim hierarchy")
            state.select_option(label="Melaka"); page.wait_for_timeout(350); check(len(usable_options(district))>=3,"State reset refreshes District options"); check(len(usable_options(pbt))==4,"State reset refreshes Melaka PBT catalogue")
            ...
            for ident in ["landuse1","landuse2","landuse3"]: page.locator(f"#{ident}").evaluate("""el=>{let p=el;while(p&&!(p.classList&&p.classList.contains('sec')))p=p.parentElement;if(p?.classList.contains('collapsed'))p.querySelector('.sechead button')?.click();}""")
            check(page.locator("#landuse1").is_visible(),"land-use controls visible"); page.locator("#landuse1").select_option(label="Komersial"); check(len(page.locator("#landuse2 option").all_text_contents())>=2,"GT2 cascades"); page.locator("#landuse2").select_option(index=1); check(len(page.locator("#landuse3 option").all_text_contents())>=2,"GT3 cascades")
            ...
            page.locator("#landuse1").evaluate("""el=>{let p=el;while(p&&!(p.classList&&p.classList.contains('sec')))p=p.parentElement;p?.querySelector('.sechead button')?.click();}"""); page.wait_for_timeout(100)
            sec_after=page.locator("#landuse1").evaluate("""el=>{let p=el;while(p&&!(p.classList&&p.classList.contains('sec')))p=p.parentElement;return p?.classList.contains('collapsed')}"""); check(sec_after!=sec_before,"section header toggles state once")
            ...
            for layer_id in layer_ids:
                ...
                try:
                    ...
                    state_text=page.locator(f"[data-layer-state='{layer_id}']").inner_text().strip().upper(); check(not state_text.startswith("ERROR"),f"layer {layer_id} state non-error"); check(page.evaluate("id=>{const l=window.__URBION_LIVE_LAYERS__?.[id];return !!l&&map.hasLayer(l)}",layer_id),f"layer {layer_id} mounted on map"); check(page.evaluate("id=>{const l=window.__URBION_LIVE_LAYERS__?.[id];if(!l)return false;return Object.keys(l._tiles||{}).length>0||(l._container?.querySelectorAll('img,canvas').length||0)>0}",layer_id),f"layer {layer_id} rendered imagery"); layer_results.append({"id":layer_id,"ok":True,"state":state_text})
                except Exception as exc:
                    layer_results.append({"id":layer_id,"ok":False,"error":str(exc)}); failed.append(f"layer {layer_id}: {exc}")
                finally:
                    row=page.locator(f"#layerList [data-urbion-layer='{layer_id}']")
            ...
            for kind,selector in [("evidence","#evidenceBtn"),("whatif","#whatifBtn"),("decision","#decisionBtn"),("output","#outputBtn")]: page.locator(selector).click(); page.wait_for_timeout(150); check(page.locator("#modal").is_visible(),f"{kind} modal opens"); page.locator("#closeModal").click(); page.wait_for_timeout(100); check(not page.locator("#modal").is_visible(),f"{kind} modal closes")
            page.locator("#whatifBtn").click(); page.wait_for_timeout(150); check(page.locator("#modal").is_visible(),"what-if studio opens"); whatif_requests.clear(); wf_button=page.locator("#whatIfRun");
            if wf_button.count(): wf_button.click(); page.wait_for_timeout(300); check(len(whatif_requests)==1,"what-if sends exactly one request")
            page.locator("#closeModal").click(); page.wait_for_timeout(100)
            for selector,label in [("#runtimeAbout","ABOUT"),("#runtimeHelp","HELP"),("#runtimeSources","SOURCES"),("#runtimeStatus","STATUS")]: page.locator(selector).click(); page.wait_for_timeout(100); check(page.locator("#modal").is_visible(),f"{label} utility opens"); page.locator("#closeModal").click()
            ...
            for width,height in [(1440,900),(1366,768),(1920,1080)]: page.set_viewport_size({"width":width,"height":height}); page.wait_for_timeout(120); overflow=page.evaluate("document.documentElement.scrollWidth>window.innerWidth+4 || document.body.scrollWidth>window.innerWidth+4"); check(not overflow,f"responsive no horizontal overflow at {width}x{height}")
            page.set_viewport_size({"width":1440,"height":900}); resources=page.evaluate("performance.getEntriesByType('resource').map(e=>e.name)"); legacy_hits=[u for u in resources if "urbion_workspace_final" in u or "workspace_final" in u]; check(not legacy_hits,"legacy workspace assets absent"); check(not failed,"no non-GIS browser request failures"); check(not http,"no non-GIS HTTP errors"); checks.append("all canonical browser-gate controls passed")
        except Exception as exc: failed.append(str(exc)); print("CHECK FAILURES:"); print(exc)
        finally:
            page.screenshot(path=str(ARTIFACT / "workspace-final.png"), full_page=True)
            (ARTIFACT / "workspace-browser.json").write_text(json.dumps({"base":BASE,"failed":failed,"http":http,"gis_optional":gis_optional,"console_errors":console_errors,"checks":checks},indent=2),encoding="utf-8")
            browser.close()
