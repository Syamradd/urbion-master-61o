...
def main() -> None:
    ...
    with urllib.request.urlopen(request, timeout=30) as response:
        assert response.status == 200, f"unexpected /assess status: {response.status}"
    ...
    assert packet.get("version") == "PHASE1.1", "unexpected packet version"
    ...
    print(f"[PHASE1-CANONICAL] packet version={packet['version']} status={packet['assessment']['final_status']} gaps={len(packet['review_gaps'])}")
