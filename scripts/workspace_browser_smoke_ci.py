...
Loads the product smoke source, applies deterministic CI-only hardening, installs
an explicit readiness fixture, then executes the original smoke gate.
"""
...
def prepare_ready_case(page):
    ...
    if missing:
        print(f"[TRACE-CI-V5] readiness missing={missing}; values={values}; gt2={usable_options(gt2)}; gt3={usable_options(gt3)}")
