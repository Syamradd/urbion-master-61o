...
function boot(){
 ...
 try{if(localStorage.getItem('urbion-theme-final')==='light')document.body.classList.add('fcc-light');setLogo();}catch(e){}
 ...
 function translate(){
  ...
  root.querySelectorAll('label,.fcc-step b,.fcc-map-head b,.fcc-card-head span,.fcc-footer-links button,.fcc-rail .fcc-kicker,.fcc-case>.fcc-kicker,.fcc-commandbar h2,.fcc-chain span,.fcc-decision-list b,.fcc-decision-list span').forEach(el=>{
   ...
   const en=el.dataset.en;el.textContent=bm?(translations[en]||en):en;
  });
  if(theme)theme.setAttribute('aria-label',bm?'Tukar tema':'Toggle theme');
 }
 window.addEventListener('urbion:final-language',translate);translate();
 const lang=document.getElementById('fcc-lang');lang?.addEventListener('click',()=>setTimeout(translate,20));
}
