...
app=FastAPI(title='URBION API',version='MASTER-67');app.add_middleware(CORSMiddleware,allow_origins=['*'],allow_credentials=False,allow_methods=['*'],allow_headers=['*'])
...
def assess_core(r):
 ...
 sa=build_site_analysis(state=r.state,district=r.district,pbt=r.pbt,lot_no=r.lot_no,latitude=r.site_lat,longitude=r.site_lon,tod_distance_m=d,development_class=dc,development_type=r.development_type,policy_status=fs,final_status=fs);reg=source_registry_snapshot()
 return {'project':'URBION','version':'MASTER-67','site':{'latitude':r.site_lat,'longitude':r.site_lon,'state':r.state,'district':r.district,'pbt':r.pbt,'lot_no':r.lot_no or'Not specified'},'tod':{'latitude':r.tod_lat,'longitude':r.tod_lon},'precinct':r.precinct,'development_class':dc,'development_type':r.development_type,'proposal':prop,'tod_distance_m':d,'classification':cl,'policy_coverage':cov,'retrieved_rules':rr,'applicability_results':ar,'compliance_results':cr,'final_rule':fr,'final_status':fs,'site_analysis':sa,'recommendation':sa['recommendation'],'decision_confidence':sa['decision_confidence'],'source_registry':reg,'evidence_intelligence':summarise_sources(reg),'decision_trace':decision_trace(fs,rr,ar,cr),'gis_provenance':'URBION GIS decision pipeline + source registry; external portal live-query status is explicitly disclosed.'}
@app.get('/')
def root():return{'project':'URBION','version':'MASTER-67','status':'ONLINE'}
@app.get('/health')
def health():return{'status':'healthy','engine':'URBION MASTER-67'}
@app.get('/metadata')
def metadata():return{'project':'URBION HORIZON','version':'MASTER-67','states':sorted(STATE_PBT.keys()),'pbt':STATE_PBT,'development_classes':DEVELOPMENT_CLASSES,'source_registry':source_registry_snapshot(),'policy_coverage':'RT MBMB 2035 rule engine active for supported typologies; other PBTs are spatial-demo coverage only.','decision_layer':'Explainable recommendation + evidence confidence + decision trace; not statutory approval.'}
@app.get('/evidence-summary')
def evidence_summary():
 reg=source_registry_snapshot();return summarise_sources(reg)
@app.post('/assess')
