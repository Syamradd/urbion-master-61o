...
app = FastAPI(title="URBION API", version="MASTER-61O")
...
@app.get("/")
def root():
    return {"project": "URBION", "version": "MASTER-61O", "status": "ONLINE"}

@app.get("/health")
def health():
    return {"status": "healthy", "engine": "URBION MASTER-61O"}

@app.post("/assess")
def assess(request: AssessmentRequest):
    distance = distance_m(
        request.site_lat, request.site_lon,
        request.tod_lat, request.tod_lon
    )
    ...
    proposal = {
        ...
        "authority": "MBMB",
        "planning_reference": "RT MBMB 2035",
    ...
    compliance_results = urbion_evaluate_compliance(
        applicability_results, proposal
    )
    ...
    elif classification == "OUTSIDE TOD 800m" and (
        "tod" in request.development_type.lower()
        or "mixed" in request.development_type.lower()
    ):
        final_rule = None
    ...
    return {
        ...
        "tod": {
            "latitude": request.tod_lat,
            "longitude": request.tod_lon,
        },
        "precinct": request.precinct,
        "development_type": request.development_type,
        "proposal": proposal,
        ...
        "gis_provenance": "URBION GIS decision pipeline",
    }
