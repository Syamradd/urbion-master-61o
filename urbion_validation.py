...
def run_validation_case(case_id: str, assess_fn, copilot_fn) -> dict[str, Any] | None:
    ...
    if case_id=="TC-01":
        ...
        spatial=build_spatial_intelligence(inputs["site_lat"],inputs["site_lon"],inputs.get("tod_lat"),inputs.get("tod_lon"))
        evidence_card=_compact_assessment(result); evidence_card["spatial"]=_compact_spatial(spatial); evidence_card["validation_path"]=["ASSESSMENT","SPATIAL"]
    elif case_id=="TC-03":
        ...
        result=assess_fn(inputs); evidence_card=_compact_assessment(result); evidence_card["validation_path"]=["ASSESSMENT","POLICY","APPLICABILITY","COMPLIANCE"]
