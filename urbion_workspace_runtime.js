...
function addUtilities(){const top=$('.top');if(!top||$('runtimeTools'))return;const box=document.createElement('div');box.id='runtimeTools';box.style.cssText='display:flex;align-items:center;gap:4px;margin-left:4px';const make=(id,label)=>{const b=document.createElement('button');b.id=id;b.className='tool';b.type='button';b.textContent=label;return b};const about=make('runtimeAbout','ABOUT'),help=make('runtimeHelp','HELP'),sources=make('runtimeSources','SOURCES'),status=make('runtimeStatus','STATUS'),fs=make('runtimeFullscreen','FULLSCREEN'),reset=make('runtimeReset','RESET');[about,help,sources,status,fs,reset].forEach(b=>box.appendChild(b));$('themeBtn')?.parentNode?.insertBefore(box,$('themeBtn').nextSibling);help.onclick=()=>modal('URBION HORIZON — HELP','<div class="grid2"><div class="resultbox"><h4>CASE</h4><p>Define location, development and the Guna Tanah 1 → 2 → 3 hierarchy.</p></div><div class="resultbox"><h4>MAP</h4><p>Use Map / Satellite / Hybrid and click the map to select a site.</p></div><div class="resultbox"><h4>LAYERS</h4><p>Open Layers and switch official catalogue layers ON/OFF. Source/type state is shown alongside the layer.</p></div><div class="resultbox"><h4>ANALYSE</h4><p>Run Site Analysis to populate spatial evidence, RT/GP, evidence health and decision support.</p></div></div>');sources.onclick=()=>modal('DATA SOURCES','<div class="resultbox"><h4>PLANNING / SPATIAL</h4><p>PLANMalaysia i-Plan · official map/service context returned by the workspace catalogue · planning documents and guideline context.</p></div><div style="height:8px"></div><div class="resultbox"><h4>BASEMAP</h4><p>OpenStreetMap · Esri World Imagery · Esri reference labels.</p></div>');status.onclick=async()=>{let html='';for(const [label,url] of [['Health','/health'],['Metadata','/metadata'],['Map catalogue','/map/layers?state='+encodeURIComponent($('state')?.value||'Melaka')]]){try{const r=await fetch(url,{cache:'no-store'});html+=`<div class="mini"><span>${label}</span><span class="tag ${r.ok?'ok':'bad'}">${r.ok?'ONLINE':'ERROR '+r.status}</span></div>`}catch(e){html+=`<div class="mini"><span>${label}</span><span class="tag bad">ERROR</span></div>`}}modal('SYSTEM STATUS',html)};fs.onclick=async()=>{try{if(!document.fullscreenElement)await document.documentElement.requestFullscreen();else await document.exitFullscreen()}catch(e){toast('Fullscreen unavailable',false)}};reset.onclick=()=>{if(confirm('Reset this planning case?'))location.reload()}}
