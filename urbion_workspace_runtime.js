...
async function boot(){for(let i=0;i<160;i++){try{if(window.URBION_FINAL){try{window.URBION_FINAL.refreshMap?.()}catch(_){ }try{window.URBION_FINAL.loadLayers?.()}catch(_){ }for(let pass=0;pass<24;pass++){safeDefaults();compactRightRail();ensurePlanningLayers();await sleep(250)}return}}catch(_){ }await sleep(50)}console.warn('URBION compatibility runtime: canonical bridge unavailable')}
