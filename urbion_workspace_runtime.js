/* URBION HORIZON — canonical runtime takeover.
   Purpose: remove competing handlers only from controls owned by this runtime,
   while preserving canonical case-builder handlers. */
(()=>{
'use strict';
if(window.__URBION_RUNTIME_CANONICAL_V4__) return;
window.__URBION_RUNTIME_CANONICAL_V4__=true;
const $=id=>document.getElementById(id);
const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
function waitForCore(done){let n=0;const tick=()=>{if(window.URBION_FINAL){done();return}if(n++<160){setTimeout(tick,100)}else console.warn('URBION_FINAL not available')};tick()}
...
function modal(title,html){const m=$('modal');if(!m)return;const h=$('modalTitle'),b=$('modalBody');if(h)h.textContent=title;if(b)b.innerHTML=html;m.classList.add('show')}
...
function takeoverControls(){
  const owned=[
    ...
    ...document.querySelectorAll('#layerBtn,#run,#evidenceBtn,#whatifBtn,#decisionBtn,#outputBtn,#generateOutput,#printBtn,#closeModal,#useMap,#locate,#ring400,#ring800,#ring1000'),
    ...document.querySelectorAll('#roadCtx,#roadCtxMap,#context400,#context800,#context1000,#themeBtn,#langBtn,#search')
  ];
  [...new Set(owned)].forEach(replaceNode);
  document.querySelectorAll('.sec .sechead button').forEach(b=>{
    if(!b.dataset.urbionCollapseBound){
      b.dataset.urbionCollapseBound='1';
      b.addEventListener('click',()=>b.parentElement.parentElement.classList.toggle('collapsed'));
    }
  });
}
function setBasemap(kind){if(typeof map==='undefined'||typeof baseLayers==='undefined'||!map||!baseLayers[kind])return;Object.values(baseLayers).forEach(layer=>{try{if(map.hasLayer(layer))map.removeLayer(layer)}catch(_){}});baseLayers[kind].addTo(map);document.querySelectorAll('[data-base]').forEach(b=>b.classList.toggle('active',b.dataset.base===kind));if($('mapStatus'))$('mapStatus').textContent=(kind==='street'?'MAP':kind==='sat'?'SATELLITE':'HYBRID')+' · LIVE';map.invalidateSize(true);toast((kind==='street'?'Map':kind==='sat'?'Satellite':'Hybrid')+' base active')}
function setupLandUse(){
  ...
  badge.textContent='PLANMalaysia · Manual GIS Rancangan Pemajuan Versi 3 (2025) · GT1 → GT2 → GT3';
}
function bindMapControls(){
  ...
  $('locate')?.addEventListener('click',()=>{const lat=Number($('site_lat')?.value),lon=Number($('site_lon')?.value);if(Number.isFinite(lat)&&Number.isFinite(lon)&&typeof map!=='undefined'&&map){map.setView([lat,lon],Math.max(map.getZoom(),14));if(typeof marker!=='undefined'&&marker)marker.setLatLng([lat,lon]);[typeof r400!=='undefined'?r400:null,typeof r800!=='undefined'?r800:null,typeof r1000!=='undefined'?r1000:null].forEach(x=>x?.setLatLng([lat,lon]));}toast('Map recentered to selected site')});
  ['context400','context800','context1000'].forEach((id,i)=>$(id)?.addEventListener('change',e=>{const obj=[typeof r400!=='undefined'?r400:null,typeof r800!=='undefined'?r800:null,typeof r1000!=='undefined'?r1000:null][i];if(obj&&typeof map!=='undefined'&&map){e.target.checked?obj.addTo(map):obj.removeFrom(map)}}));
  ...
  $('roadCtx')?.addEventListener('change',e=>{if(typeof map==='undefined'||!map||typeof L==='undefined')return;if(!window.__urbionRoadLayer)window.__urbionRoadLayer=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{opacity:.42,maxZoom:19,attribution:'© OpenStreetMap'});e.target.checked?window.__urbionRoadLayer.addTo(map):map.removeLayer(window.__urbionRoadLayer)});
...
async function searchBinding(){
  const s=$('search');if(!s)return;const holder=s.parentElement;holder.style.position='relative';const existing=$('runtimeSearchResults');if(existing)existing.remove();const box=document.createElement('div');box.id='runtimeSearchResults';box.style.cssText='position:absolute;left:0;right:0;top:39px;z-index:1200;display:none;max-height:220px;overflow:auto;background:#061820;border:1px solid var(--line);border-radius:9px;padding:6px';holder.appendChild(box);let timer;
  const query=async q=>{if(q.length<3){box.style.display='none';return}try{const r=await fetch('https://nominatim.openstreetmap.org/search?format=jsonv2&limit=5&q='+encodeURIComponent(q),{headers:{Accept:'application/json'}});if(!r.ok)throw new Error('search HTTP '+r.status);const d=await r.json();box.innerHTML=d.length?d.map((x,i)=>`<button type="button" data-i="${i}" style="display:block;width:100%;text-align:left;background:transparent;border:0;color:inherit;padding:7px;font-size:8px;border-bottom:1px solid rgba(45,85,100,.18)">${esc(x.display_name)}</button>`).join(''):'<div style="padding:7px;font-size:8px;color:var(--muted)">No location found</div>';box.style.display='block';box.querySelectorAll('[data-i]').forEach(btn=>btn.onclick=()=>{const x=d[+btn.dataset.i],lat=+x.lat,lon=+x.lon;$('site_lat').value=lat.toFixed(6);$('site_lon').value=lon.toFixed(6);$('coords').textContent=`${lat.toFixed(6)}, ${lon.toFixed(6)}`;if(typeof map!=='undefined'&&map){map.setView([lat,lon],Math.max(map.getZoom(),14));if(typeof marker!=='undefined'&&marker)marker.setLatLng([lat,lon]);[typeof r400!=='undefined'?r400:null,typeof r800!=='undefined'?r800:null,typeof r1000!=='undefined'?r1000:null].forEach(r=>r?.setLatLng([lat,lon]))}s.value=x.display_name;box.style.display='none';toast('Location selected')})}catch(e){box.innerHTML='<div style="padding:7px;font-size:8px;color:var(--bad)">Search unavailable</div>';box.style.display='block'}};
...
async function runAnalysisWithAI(){
  try{
    ...
    const host=$('findings');
    if(host){host.querySelector('#aiSynthesisCard')?.remove();const ai=document.createElement('div');ai.className='finding';ai.id='aiSynthesisCard';ai.innerHTML=`<b>AI PLANNING SYNTHESIS</b><p>${esc(text)}</p><small>${esc(provider)} · ${esc(status)} · DETERMINISTIC PACKET REMAINS SOURCE OF TRUTH</small>`;host.appendChild(ai);}
    if($('evidenceHealth')){$('evidenceHealth').querySelector('#aiEvidenceCard')?.remove();$('evidenceHealth').insertAdjacentHTML('beforeend',`<div class="finding" id="aiEvidenceCard"><b>AI / COPILOT</b><p>Planner narrative generated from the deterministic planning packet and evidence ledger.</p><small>${esc(provider)} · ${esc(status)} · NO STATUTORY AUTHORITY</small></div>`);}
...
function addUtilities(){const top=document.querySelector('.top');if(!top||$('runtimeTools'))return;const box=document.createElement('div');box.id='runtimeTools';box.style.cssText='display:flex;align-items:center;gap:4px;margin-left:4px;flex-shrink:0';const make=(id,label)=>{const b=document.createElement('button');b.id=id;b.className='tool';b.type='button';b.textContent=label;return b};const about=make('runtimeAbout','ABOUT'),help=make('runtimeHelp','HELP'),sources=make('runtimeSources','SOURCES'),status=make('runtimeStatus','STATUS'),fs=make('runtimeFullscreen','FULLSCREEN'),reset=make('runtimeReset','RESET');[about,help,sources,status,fs,reset].forEach(b=>box.appendChild(b));top.appendChild(box);
  const act=(btn,fn)=>{btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();try{fn()}catch(err){toast(String(err.message||err),false)}})};
  act(about,()=>{location.href='/about'});
  act(help,()=>modal('URBION HORIZON — HELP','<div class="grid2"><div class="resultbox"><h4>CASE</h4><p>Define location, development and the Guna Tanah 1 → 2 → 3 hierarchy.</p></div><div class="resultbox"><h4>MAP</h4><p>Use Map / Satellite / Hybrid and click the map to select a site.</p></div><div class="resultbox"><h4>LAYERS</h4><p>Open Layers and switch official catalogue layers ON/OFF. Source/type state is shown alongside the layer.</p></div><div class="resultbox"><h4>ANALYSE</h4><p>Run Site Analysis to populate spatial evidence, RT/GP, evidence health and decision support.</p></div></div>'));
  act(sources,()=>modal('DATA SOURCES','<div class="resultbox"><h4>PLANNING / SPATIAL</h4><p>PLANMalaysia i-Plan · official map/service context returned by the workspace catalogue · planning documents and guideline context.</p></div><div style="height:8px"></div><div class="resultbox"><h4>BASEMAP</h4><p>OpenStreetMap · Esri World Imagery · Esri reference labels.</p></div>'));
  act(status,async()=>{let html='';for(const [label,url] of [['Health','/health'],['Metadata','/workstation/metadata'],['Map catalogue','/map/layers?state='+encodeURIComponent($('state')?.value||'Melaka')]]){try{const r=await fetch(url,{cache:'no-store'});html+=`<div class="mini"><span>${label}</span><span class="tag ${r.ok?'ok':'bad'}">${r.ok?'ONLINE':'ERROR '+r.status}</span></div>`}catch(e){html+=`<div class="mini"><span>${label}</span><span class="tag bad">ERROR</span></div>`}}modal('SYSTEM STATUS',html)});
  act(fs,async()=>{try{if(!document.fullscreenElement)await document.documentElement.requestFullscreen();else await document.exitFullscreen()}catch(e){toast('Fullscreen unavailable',false)}});
  act(reset,()=>{if(confirm('Reset this planning case?'))location.reload()});
}
