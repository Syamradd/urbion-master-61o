...
def haversine_m(lat1, lon1, lat2, lon2):
    lat1, lon1 = _coords(lat1, lon1)
    lat2, lon2 = _coords(lat2, lon2)
...
def midpoint(lat1, lon1, lat2, lon2):
    ...
    p1, p2 = math.radians(lat1), math.radians(lat2)
    dl = math.radians(lon2 - lon1)
    x = math.cos(p2) * math.cos(dl)
    y = math.cos(p2) * math.sin(dl)
    p3 = math.atan2(math.sin(p1) + math.sin(p2), math.sqrt((math.cos(p1) + x) ** 2 + y ** 2))
    l3 = math.radians(lon1) + math.atan2(y, math.cos(p1) + x)
...
def destination(lat, lon, distance_m, bearing):
    ...
    theta = math.radians(float(bearing) % 360)
...
def circle_geojson(lat, lon, radius_m, segments=72):
    ...
    segments = max(24, min(180, int(segments)))
    ring = []
    for i in range(segments + 1):
        p = destination(lat, lon, radius_m, i * 360.0 / segments)
        ring.append([p["longitude"], p["latitude"]])
...
def catchment_features(lat, lon, radii=(400, 800)):
    lat, lon = _coords(lat, lon)
    radii = tuple(sorted({int(r) for r in radii if int(r) > 0})) or (400, 800)
...
def nearest_feature(lat, lon, features):
    ...
    for index, feature in enumerate(features or []):
        if not isinstance(feature, dict):
            continue
        f_lat = feature.get("latitude", feature.get("lat"))
        f_lon = feature.get("longitude", feature.get("lon"))
        if f_lat is None or f_lon is None:
            continue
        try:
            d = haversine_m(lat, lon, f_lat, f_lon)
        except (TypeError, ValueError):
            continue
        ranked.append((d, index, feature))
    if not ranked:
        return None
    d, index, feature = min(ranked, key=lambda x: x[0])
    return {"index": index, "distance_m": round(d, 2), "feature": feature, "evidence": "CALCULATED"}
...
def constraint_summary(constraints=None):
    ...
    for key, raw in (constraints or {}).items():
        if isinstance(raw, bool):
            state = "CONSTRAINT_PRESENT" if raw else "NOT_FLAGGED"
        elif raw in (None, "", [], {}):
            state = "NOT_ASSESSED"
        else:
            state = "CONTEXT_REPORTED"
...
def _environment_signals(environmental_context):
    """Summarize queried environmental layers without converting source context into approval."""
    if not isinstance(environmental_context, dict):
        return [], ["Environmental layer evidence was not supplied; environmental impact remains unverified."]
    layers = environmental_context.get("layers")
    ...
    for key, layer in layers.items():
        ...
        if status == "LIVE_QUERY":
            signals.append({"id": f"environment_{key}", "value": count if count is not None else 0, "unit": "features", "classification": "SOURCE_CONTEXT_HIT", "evidence": "SOURCE_CONTEXT", "decision_use": "ENVIRONMENTAL_SCREENING"})
        elif status == "NO_FEATURE":
            signals.append({"id": f"environment_{key}", "value": 0, "unit": "features", "classification": "SOURCE_CONTEXT_NO_FEATURE", "evidence": "SOURCE_CONTEXT", "decision_use": "ENVIRONMENTAL_SCREENING"})
        else:
            signals.append({"id": f"environment_{key}", "value": status, "classification": "QUERY_GAP", "evidence": "UNVERIFIED", "decision_use": "REVIEW_PRIORITY"})
            gaps.append(f"Environmental layer {key} could not be established as a usable query result.")
    if signals:
        gaps.append("Environmental layer results remain source context; confirm currency, geometry, thresholds and agency requirements.")
    return signals, gaps


def _planning_signals(result, environmental_context=None):
    signals = []
    tod = result.get("tod")
    if tod:
    ...
    env_signals, _ = _environment_signals(environmental_context)
    signals.extend(env_signals)
    return signals


def build_spatial_intelligence(site_lat, site_lon, tod_lat=None, tod_lon=None, radii=(400, 800), constraints=None, environmental_context=None):
    site_lat, site_lon = _coords(site_lat, site_lon)
    result = {
        "site": {"latitude": site_lat, "longitude": site_lon},
        "catchments": catchment_features(site_lat, site_lon, radii),
        "constraints": constraint_summary(constraints),
        "evidence_model": {"geometry": "CALCULATED", "constraints": "USER_PROVIDED", "authoritative_overlay": "SOURCE_CONTEXT ONLY"},
    }
    if environmental_context is not None:
        result["environment"] = environmental_context
        result["evidence_model"]["environmental_overlay"] = "SOURCE_CONTEXT ONLY"
    if tod_lat is not None or tod_lon is not None:
        if tod_lat is None or tod_lon is None:
            raise ValueError("tod_lat and tod_lon must be provided together")
    ...
    result["planning_signals"] = _planning_signals(result, environmental_context)
    result["review_gaps"] = ["Authoritative spatial overlays remain source context until verified."]
    _, env_gaps = _environment_signals(environmental_context)
    result["review_gaps"].extend(env_gaps)
    ...
    result["review_gaps"] = list(dict.fromkeys(result["review_gaps"]))
    return result
