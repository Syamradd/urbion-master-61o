...
const CORE_LAYER_IDS=new Set(['iplan-current','iplan-zoning','iplan-committed','iplan-rfn','iplan-rsn','iplan-flood','iplan-disaster-risk','iplan-ksas','iplan-cfs','iplan-ecology','iplan-heritage','iplan-affordable-housing','iplan-topography','iplan-hutan','iplan-coastal-erosion','iplan-rmm','iplan-contours','mygems-faults','mygems-quarries','mygems-groundwater','mygems-geowarisan','mygems-lithology','mygems-seismic','mygems-mineral']);
function getMap(){try{const m=(typeof window.map!=='undefined'&&window.map)||window.__URBION_MAP__||window.URBION_FCC_MAP||null;if(m&&typeof m.addLayer==='function'&&typeof m.hasLayer==='function')return m;const fallback=(typeof map!=='undefined'&&map)||null;return fallback&&typeof fallback.addLayer==='function'?fallback:null}catch(_){try{const fallback=(typeof map!=='undefined'&&map)||null;return fallback&&typeof fallback.addLayer==='function'?fallback:null}catch(__){return null}}}
...
async function catalog(){const r=await fetch(location.origin+'/map/layers?state='+encodeURIComponent(state())+'&_='+Date.now(),{cache:'no-store'});if(!r.ok)throw Error(`HTTP ${r.status}`);const d=await r.json();const incoming=Array.isArray(d.layers)?d.layers:[];const all=[cadastral(),...incoming.filter(x=>x&&CORE_LAYER_IDS.has(x.id))];const seen=new Set();return all.filter(x=>x&&(!seen.has(x.id)&&(seen.add(x.id),true)))}
function clearLive(){const m=getMap(),store=window.__URBION_LIVE_LAYERS__||{};Object.values(store).forEach(l=>{try{if(m&&m.hasLayer(l))m.removeLayer(l)}catch(_){}});window.__URBION_LIVE_LAYERS__={}}
function addLayerRow(body,x){const row=document.createElement('div');row.className='layerrow';row.dataset.urbionLayer=x.id;const cb=document.createElement('input');cb.type='checkbox';cb.dataset.urbionLayer=x.id;const name=document.createElement('span');name.textContent=x.name||x.id;const source=document.createElement('small');source.textContent=x.source||x.type||'';row.append(cb,name,source);const st=document.createElement('div');st.className='layerstate';st.dataset.layerState=x.id;st.dataset.state='off';st.textContent='OFF · '+String(x.type||'SOURCE');body.append(row,st);return {row,cb,st}}
function draw(layers){const list=$('layerList');if(!list)return;list.replaceChildren();const tools=document.createElement('div');tools.className='layer-tools';tools.innerHTML='<button type="button" data-layer-tool="off">CLEAR MAP</button><button type="button" data-layer-tool="refresh">REFRESH</button>';list.appendChild(tools);const groups={};layers.forEach(x=>(groups[x.group||'OTHER']??=[]).push(x));const order=['PLANNING','CADASTRAL','TRANSPORT','HAZARD','ENVIRONMENT','HYDROLOGY','ECOLOGY','GEOLOGY','GEOHAZARD','GEOHERITAGE','HERITAGE','HOUSING','TERRAIN'];[...order.filter(k=>groups[k]),...Object.keys(groups).filter(k=>!order.includes(k))].forEach((g,idx)=>{const wrap=document.createElement('div');wrap.className='layer-group'+(idx?' closed':'');const h=document.createElement('button');h.type='button';h.className='layer-group-head';h.textContent=g.replaceAll('_',' ');h.onclick=()=>wrap.classList.toggle('closed');const body=document.createElement('div');body.className='layer-group-body';groups[g].forEach(x=>{const built=addLayerRow(body,x);built.cb.addEventListener('change',()=>toggle(x,built.cb.checked))});wrap.append(h,body);list.appendChild(wrap)});list.querySelector('[data-layer-tool="off"]')?.addEventListener('click',()=>{clearLive();list.querySelectorAll('[data-urbion-layer]').forEach(c=>c.checked=false);layers.forEach(x=>setState(x.id,'OFF · '+String(x.type||'SOURCE'),'off'))});list.querySelector('[data-layer-tool="refresh"]')?.addEventListener('click',()=>void refresh())}
function toggle(info,on){const m=getMap();if(!m||!info)return;window.__URBION_LIVE_LAYERS__=window.__URBION_LIVE_LAYERS__||{};const old=window.__URBION_LIVE_LAYERS__[info.id];if(!on){if(old&&m.hasLayer(old))m.removeLayer(old);delete window.__URBION_LIVE_LAYERS__[info.id];setState(info.id,'OFF · '+String(info.type||'SOURCE'),'off');return}if(old&&m.hasLayer(old)){setState(info.id,'ON · RENDERED','ok');return}try{setState(info.id,'LOADING · SOURCE','loading');const layer=makeLayer(info);if(!layer){setState(info.id,'PORTAL · SOURCE','off');return}layer.addTo(m);window.__URBION_LIVE_LAYERS__[info.id]=layer;setTimeout(()=>{const e=document.querySelector(`[data-layer-state="${CSS.escape(info.id)}"]`);if(m.hasLayer(layer)&&e?.dataset.state==='loading')setState(info.id,'ON · RENDERED','ok')},5000)}catch(e){setState(info.id,'ERROR · '+e.message,'error');console.error('URBION authoritative layer failure',info,e)}}
async function refresh(){const m=getMap();if(!m||typeof L==='undefined'||!$('layerList'))return false;try{draw(await catalog());normalizeEnglishLabels();return true}catch(e){$('layerList').innerHTML=`<div class="tiny" style="color:var(--bad)">Live catalogue error · ${String(e.message||e)}</div>`;return false}}
async function boot(){addCss();normalizeEnglishLabels();setupAccordions();for(let i=0;i<120;i++){if(getMap()&&typeof L!=='undefined'&&$('layerList')){await refresh();normalizeEnglishLabels();return}await sleep(250)}}
