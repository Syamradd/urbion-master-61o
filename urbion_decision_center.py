...
def build_decision_center(*, assessment: dict[str, Any], evidence: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    """Compose decision, evidence and map layers without changing statutory outcomes."""
    ...
    return {"project":"URBION HORIZON","version":"PHASE-E.5","decision":{"status":status,"recommendation":assessment.get("recommendation", {}),"confidence":assessment.get("decision_confidence", {}),"suitability":analysis.get("suitability_score")},"site":site,"planning_value":assessment.get("planning_value", {}),"evidence":evidence_summary,"decision_trace":assessment.get("decision_trace", []),"map":decision_map_payload([feature]),"guardrail":"Decision support only; evidence gaps remain disclosed and no statutory approval is inferred."}
