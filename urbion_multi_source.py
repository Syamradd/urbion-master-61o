...
def build_spatial_intelligence(*, source_context: dict | None = None,
                               ...
                               flood_exposure: bool | None = None,
                               include_domains: bool = False) -> dict:
    ...
    if include_domains:
        for metric_id, domain in (
            ("slope", "terrain"),
            ("environment", "doe-myeqms"),
            ("geohazard", "jmg-mygems"),
            ("cadastral", "jupem-mylot"),
            ("ecology", "iplan"),
        ):
            metrics.append({"id": metric_id, "value": None, "evidence": "SOURCE_CONTEXT" if domain in ctx else "UNVERIFIED", "status": "SOURCE_CONTEXT" if domain in ctx else "REVIEW_REQUIRED", "source": domain})
