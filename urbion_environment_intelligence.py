...
def _normalise(item: Any) -> tuple[Any, str, str | None, dict[str, Any]]:
    if isinstance(item, dict):
        return item.get("value"), str(item.get("evidence", "UNVERIFIED")), item.get("source"), item
    if item is None:
...
def _derive_from_planmalaysia(context: dict[str, Any]) -> dict[str, Any]:
    ...
    def hit(key: str) -> bool:
        return bool((layers.get(key) or {}).get("feature_count", 0))

    if "flood" in layers:
        out["flood"] = {"value": hit("flood"), "evidence": "SOURCE_CONTEXT", "source": "PLANMalaysia DPFDN — Banjir 100 tahun", "query": layers["flood"]}
    if "ksas" in layers:
        out["ksas"] = {"value": hit("ksas"), "evidence": "SOURCE_CONTEXT", "source": "PLANMalaysia DPFDN — KSAS", "query": layers["ksas"]}
    if "geohazard" in layers:
        out["geohazard"] = {"value": hit("geohazard"), "evidence": "SOURCE_CONTEXT", "source": "PLANMalaysia DPFDN — Tanah Runtuh", "query": layers["geohazard"]}
    if "seismic" in layers:
        out["seismic"] = {"value": hit("seismic"), "evidence": "SOURCE_CONTEXT", "source": "PLANMalaysia DPFDN — Risiko Gempa Bumi", "query": layers["seismic"]}
    if "quarry_mining" in layers:
        out["quarry_mining"] = {"value": hit("quarry_mining"), "evidence": "SOURCE_CONTEXT", "source": "JMG MyGEMS", "query": layers["quarry_mining"]}
    if "ecology" in layers:
        out["ecology"] = {"value": hit("ecology"), "evidence": "SOURCE_CONTEXT", "source": "PLANMalaysia DPFDN — CFS", "query": layers["ecology"]}
    if "river" in layers:
        out["river_reserve"] = {"value": hit("river"), "evidence": "SOURCE_CONTEXT", "source": "PLANMalaysia DPFDN — Sungai", "query": layers["river"]}
...
def build_environment_intelligence(context: dict[str, Any] | None = None) -> dict[str, Any]:
    context = context or {}
    merged = dict(context)
    for key, value in _derive_from_planmalaysia(context).items():
    ...
    evidence_counts = {
        "source_context": sum(m["evidence"] == "SOURCE_CONTEXT" for m in metrics),
        "user_provided": sum(m["evidence"] == "USER_PROVIDED" for m in metrics),
        "unverified": sum(m["evidence"] == "UNVERIFIED" for m in metrics),
    }
    evidence_sources = sorted({m["source"] for m in metrics if m["value"] is not None and m["evidence"] != "UNVERIFIED"})
    return {
        "version": "MASTER-226",
        "status": "RISK_FLAGGED" if flagged else ("PARTIALLY_SCREENED" if known else "EVIDENCE_REQUIRED"),
        "metrics": metrics,
        "summary": {"domain_count": len(metrics), "screened_count": len(known), "flagged_count": len(flagged), "review_gap_count": len(gaps)},
        "review_gaps": gaps,
        "sources": sorted({m["source"] for m in metrics}),
        "evidence_quality": {"counts": evidence_counts, "evidence_sources": evidence_sources},
        "site": context.get("site"),
        "radius_m": context.get("radius_m"),
        "decision_boundary": "ENVIRONMENTAL_SCREENING_SUPPORT",
        "statutory_verification": "NOT_CLAIMED",
        "disclaimer": "Environmental/geohazard screening is decision support only. A spatial hit within a configured radius is not a legal setback or statutory determination; confirm authoritative currency, technical thresholds and agency requirements.",
    }
