...
def _timestamp(value: Any) -> str | None:
    try:
        number = float(value)
        # APIMS/MyEQMS exposes DATETIME as Unix epoch milliseconds.
...
def normalize_feature(feature: dict[str, Any]) -> dict[str, Any] | None:
    ...
    # Preserve pollutant fields only when the authoritative response supplies them.
    aliases = {
...
def fetch_myeqms_stations(
    *,
    ...
    ``http_get`` is injectable for deterministic CI. The default transport uses
    the official DOE ArcGIS query endpoint and requests station geometry.
    """
    params = {
        "f": "json",
        "where": "1=1",
