...
def run_agents(*, assessment: dict[str, Any], spatial: dict[str, Any] | None = None,
    ...
    outputs = [
        ...
        _result("SPATIAL", "COMPLETE", spatial or assessment.get("spatial_intelligence") or {}, evidence="CALCULATED"),
        _result("POLICY", "COMPLETE" if assessment.get("retrieved_rules") else "REVIEW_REQUIRED", {"rule_count": len(assessment.get("retrieved_rules", []) or []), "coverage": assessment.get("policy_coverage")}, evidence="SOURCE_CONTEXT" if assessment.get("retrieved_rules") else "UNVERIFIED"),
        ...
        _result("IMPACT", "REVIEW_REQUIRED", {"domains": ["social", "environment", "mobility", "economic"], "reason": "Impact evidence must be supplied or queried before a material impact conclusion."}, evidence="UNVERIFIED"),
