...
def _wmts_xyz(params: dict[str, str]) -> tuple[int, int, int] | None:
    ...
    tile_span = (world * 2.0) / (2 ** z); x = int(math.floor((xmin + world) / tile_span + 1e-9)); y = int(math.floor((world - ymax) / tile_span + 1e-9))
    return z, x, y
...
async def _jmg_feature_image_fallback(parsed_path: str, params: dict[str, str]) -> Response | None:
    ...
    common = {"where": "1=1", "outFields": "OBJECTID,Line_code,Type,Name", "returnGeometry": "true", "outSR": "3857", "geometryType": "esriGeometryEnvelope", "inSR": "3857", "spatialRel": "esriSpatialRelIntersects", "geometry": bbox_raw, "f": "json"}
