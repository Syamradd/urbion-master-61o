...
const installGetter=()=>{
 const current=window.getInputs;
 if(current&&current.__URBION_VISIBLE_INPUT_GETTER_V2__)return true;
 previous=typeof current==='function'?current:null;
 wrapper=function(){let base={};try{if(previous&&previous!==wrapper)base=previous()||{}}catch(_){base={}}return {...base,...visible()}};
 wrapper.__URBION_VISIBLE_INPUT_GETTER_V2__=true;
 try{window.getInputs=wrapper}catch(_){try{Object.defineProperty(window,'getInputs',{configurable:true,writable:true,value:wrapper})}catch(__){return false}}
 return window.getInputs===wrapper;
};
...
const watch=()=>{installGetter();const ready=document.getElementById('udcDemoState')||document.querySelector('.udc-status');if(!ready)return setTimeout(watch,200);hydrate()};
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',watch,{once:true});else watch();
window.addEventListener('load',installGetter,{once:true});
const guard=window.setInterval(()=>{if(installGetter())return},50);window.setTimeout(()=>clearInterval(guard),30000);
})();
